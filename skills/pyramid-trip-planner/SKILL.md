...
```json
{
  ...
  "wishlist": [
    {
      "item": "Great Barrier Reef", "status": "confirmed", "note": "optional context",
      "lat": -18.29, "lng": 147.7, "leg": "Melbourne",
      "photo": {"url": "https://upload.wikimedia.org/...", "credit": "Wikipedia", "page_url": "https://en.wikipedia.org/wiki/..."}
    }
  ],
  "agent_notes": [
...
- **`architecture.legs`** — only populate once a Big Picture path is locked (Phase 2+, filled in during Phase 3 — see above on choosing leg granularity). `order` drives the route line on the map, so keep it sequential. Each leg's `name` is the identifier every other `leg` field below must match exactly, character for character — that's what makes a note's tag actually link back to this leg on the dashboard. The map only draws numbered pins and directional arrows once *every* plotted leg has an explicit `order` — a partially-ordered set of legs draws as a plain unnumbered route instead, since a half-locked sequence can't be trusted to imply a real direction.
- **`photo`** (optional, on `wishlist[]` items and `architecture.legs[]`) — see "Location photos" below for how to source one responsibly. Always `{"url", "credit", "page_url"}` together; never just a bare URL, since the credit/link is what makes using someone else's photo okay.
- **`start_date` / `end_date`** (optional, on `architecture.legs[]`, ISO `YYYY-MM-DD`) — only add these once the user has actually locked specific calendar dates for that leg, not the moment nights-per-leg is decided. A leg can have `nights` without dates for a while (the shape is settled, the calendar isn't) — that's normal and expected mid-Phase-3. See "Trip calendar" below for what setting these unlocks.
...
Nights-per-leg (`architecture.legs[].nights`) tells you the *shape* of the trip; it doesn't tell you the *dates*. Those are a separate, later lock-in — the user might settle on "5 nights in Osaka" well before they know it's specifically April 2-6, and the two shouldn't be conflated. The moment a leg's actual dates firm up, set `start_date`/`end_date` on that leg (ISO `YYYY-MM-DD`) — that's the only trigger the calendar needs. A calendar section appears above the map automatically once at least one leg has both dates; before that, it shows a placeholder rather than guessing.

The calendar renders a real month grid (or several, spanning however many months the trip covers), shading each locked leg's date range in that leg's own color — the same color used for its map pins and corkboard tags, so a user can visually connect a calendar block to a place without re-reading labels. It shares the leg filter with the corkboard and map: hiding a leg dims its calendar days too, everything narrows together.

**Export**: an "Export to calendar (.ics)" button turns every dated leg into a real calendar event and downloads a standard `.ics` file — importable into Google Calendar, Apple Calendar, Outlook, anything that reads iCalendar. This works with zero setup (no API, no account) because `.ics` is just a text format the browser generates client-side from the dates already in state; there's nothing you need to do to enable it beyond setting `start_date`/`end_date`. The button appears as soon as there's at least one dated leg — it doesn't have to wait for every leg to be dated, since the user may want to export a partial calendar early and add to it later.

**Map export, and why it isn't "push to Google Maps"**: the "Open route in Google Maps ↗" button next to the map (visible once every leg has a locked `order` — same gate as the arrows/numbers) deep-links into Google's public directions URL scheme with each leg as an ordered stop. This is a live, real, working route the user can open with one click — but it is *not* the same as writing pins into their own Google My Maps. Google has no public API for that; My Maps is a manual-import-only consumer product. Don't imply otherwise to the user — "open the route" is the honest framing, not "save to your Google Maps."

## Location photos

A place described only in words is harder to weigh against alternatives than one you can also see. When a wishlist item or leg is worth comparing on its own merits — a candidate the user is actually deciding between, or a leg you're describing in any depth — look up one representative photo rather than describing it in prose alone.

**Source: Wikipedia's public summary API**, not a generic image search. Fetch `https://en.wikipedia.org/api/rest_v1/page/summary/<Page_Title>` (spaces become underscores) — no API key, and it returns `thumbnail.source` (a Wikimedia Commons-hosted image, consistently freely licensed) alongside `content_urls.desktop.page` (the canonical article, for attribution). That combination is what makes embedding someone else's photo okay: a real source, a real link back. Store both as the item's `photo` field: `{"url": thumbnail.source, "credit": "Wikipedia", "page_url": content_urls.desktop.page}`. The renderer handles showing it with the credit/link intact on both the corkboard card and the map popup — you don't need to do anything else once the field is set.

If Wikipedia has nothing useful for something obscure (a specific trailhead, a small restaurant), don't force a match from a generic search — a wrong or dubiously-licensed photo is worse than no photo. Skip it, or note in `agent_notes` that a photo wasn't available.

**Don't refetch what's already there.** Check for an existing `photo` field before looking one up again — this is a one-time lookup per item, not something to redo every turn.

**Say the caveat once, early, and mean it**: a photo is a visual aid for comparing options, not a substitute for the user's own research before booking or committing. It may be an old photo, a flattering angle, a different season, or simply not represent what they'll actually experience — make that explicit the first time you show photos, not as boilerplate on every single one.
